...
learn_underscore.any = function(array, fun) {
  // YOUR CODE HERE
var arrTf = _.map(array, function(item) {
    return !!fun(item)
  });
  function tf ( bool1, bool2){
    return bool1 || bool2 ? true: false;
  }
  return _.reduce(arrTf, tf)
}
...
//  });
  var nObj = {};
  var keyList = Object.keys(arr);
  for(var i =0; i<keyList.length; i++){
    nObj[keyList[i]] = arr[keyList[i]].length
  }
  return nObj;
}

  // _.forEach(arr, function(value, key){
  //   newArr.value = key.length
  // })
  // return newArr
